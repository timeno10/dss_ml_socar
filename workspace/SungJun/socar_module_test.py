...
class SocarML:
    ...
    def split_dataset(self):
        ...
        self.X_train = self.train_data.drop('fraud_YN', axis=1)
        self.y_train = self.train_data.fraud_YN
    ...
    def get_result_pd(self):
        models = [(self.lg_clf,self.lg_params), (self.dt_clf,self.dt_params), (self.rf_clf,self.rf_params), 
        ...
        for model, param in models:
            ...
            pred_train = grid.predict(self.X_train)
            
            # predict test dataset
            pred_test = grid.predict(self.X_test)
            
            tmp.append(self.get_clf_eval(self.y_train, pred_train) + self.get_clf_eval(self.y_test, pred_test))
